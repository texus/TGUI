...
#endif
#if TGUI_HAS_BACKEND_SDL_GLES2 || TGUI_HAS_BACKEND_SDL_OPENGL3 || TGUI_HAS_BACKEND_SDL_TTF_GLES2 || TGUI_HAS_BACKEND_SDL_TTF_OPENGL3 \
    || TGUI_HAS_BACKEND_SDL_RENDERER || TGUI_HAS_BACKEND_SDL_GPU
...
struct TestsWindowSfmlOpenGL3 : public TestsWindowBase
{
    ...
    #else
    sf::Window window{sf::VideoMode{windowWidth, windowHeight},
                      windowTitle,
                      sf::Style::Default,
...
struct TestsWindowSdlRenderer : public TestsWindowBase
{
    TestsWindowSdlRenderer()
    {
        ...
        TTF_Init();
    #if SDL_MAJOR_VERSION >= 3
        window = SDL_CreateWindow(windowTitle, windowWidth, windowHeight, SDL_WINDOW_OPENGL | SDL_WINDOW_RESIZABLE);
...
int main(int argc, char* argv[])
{
    ...
    auto cli = session.cli()
               | Catch::clara::Opt(selectedBackend, "backend")["--backend"]("TGUI backend to use (empty to let TGUI pick one)");
    ...
    tgui::Timer::scheduleCallback([w = window.get()] { tgui::Timer::scheduleCallback([w] { w->close(); }); });
    globalGui->mainLoop();
