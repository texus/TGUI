// File was changed to fix -Wshadow warnings.
...
namespace Catch {
    ...
    public:
        ...
        void useStream( std::string const& streamName ) {
            Stream stream_ = createStream( streamName );
            setStreamBuf( stream_.streamBuf );
            m_stream.release();
            m_stream = stream_;
        }
...
#  endif
    ...
    class CommandLine {
        ...
        public:
            ...
            ArgBuilder& detail( std::string const& detail_ ) {
                m_arg->detail = detail_;
                return *this;
            }
    ...
    public:
        ...
        void optUsage( std::ostream& os, std::size_t indent = 0, std::size_t width = Detail::consoleWidth ) const {
            ...
            for( it = itBegin; it != itEnd; ++it ) {
                Detail::Text usage_( it->commands(), Detail::TextAttributes()
                                                        .setWidth( maxWidth+indent )
                                                        .setIndent( indent ) );
                Detail::Text desc( it->description, Detail::TextAttributes()
                                                        .setWidth( width - maxWidth - 3 ) );

                for( std::size_t i = 0; i < (std::max)( usage_.size(), desc.size() ); ++i ) {
                    std::string usageCol = i < usage_.size() ? usage_[i] : "";
                    os << usageCol;
...
namespace Catch {
    ...
    public:
        ...
        explicit RunContext( Ptr<IConfig const> const& config_, Ptr<IStreamingReporter> const& reporter )
