...
namespace tgui
{
    ...
    void Widget::rendererChanged(const String& property)
    {
        ...
        else if (property == U"TransparentTexture")
        {
            ...
            throw Exception{U"Could not set property '" + property + U"', widget of type '" + getWidgetType()
                            + U"' does not has this property."};
    }
    ...
    std::unique_ptr<DataIO::Node> Widget::save(SavingRenderersMap& renderers) const
    {
        ...
        if (getToolTip() != nullptr)
        {
            ...
            toolTipNode->propertyValuePairs[U"DistanceToMouse"] = std::make_unique<DataIO::ValueNode>(
                U"(" + String::fromNumber(ToolTip::getDistanceToMouse().x) + ","
                + String::fromNumber(ToolTip::getDistanceToMouse().y) + U")");
        ...
        if (m_parentGui)
        {
            ...
            if (node->propertyValuePairs[U"NavigationDown"])
                m_navWidgetDown = m_parentGui->get(
                    Deserializer::deserialize(ObjectConverter::Type::String, node->propertyValuePairs[U"NavigationDown"]->value)
                        .getString());
            if (node->propertyValuePairs[U"NavigationLeft"])
                m_navWidgetLeft = m_parentGui->get(
                    Deserializer::deserialize(ObjectConverter::Type::String, node->propertyValuePairs[U"NavigationLeft"]->value)
                        .getString());
            if (node->propertyValuePairs[U"NavigationRight"])
                m_navWidgetRight = m_parentGui->get(
                    Deserializer::deserialize(ObjectConverter::Type::String, node->propertyValuePairs[U"NavigationRight"]->value)
                        .getString());
        }
