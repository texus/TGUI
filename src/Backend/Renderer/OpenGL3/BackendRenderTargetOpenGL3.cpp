...
namespace tgui
{
    ...
    TGUI_NODISCARD static GLuint createShaderProgram()
    {
        ...
        if (vertexShader == 0)
            throw Exception{
                U"Failed to create shaders in BackendRenderTargetOpenGL3. glCreateShader(GL_VERTEX_SHADER) returned 0."};
        ...
        if (fragmentShader == 0)
            throw Exception{
                U"Failed to create shaders in BackendRenderTargetOpenGL3. glCreateShader(GL_FRAGMENT_SHADER) returned 0."};
        ...
        m_shaderProgram(createShaderProgram())
    {
        ...
        TGUI_ASSERT(getBackend()->getRenderer(),
                    "BackendRenderTargetOpenGL3 can't be created when there is no backend renderer (was a gui attached to a "
                    "window "
                    "yet?)");
