...
namespace
{
    [[nodiscard]] std::vector<tgui::Vector2f> drawCircleHelperGetPoints(unsigned int nrPoints, float radius, float offset)
    {
#if defined(__cpp_lib_math_constants) && (__cpp_lib_math_constants >= 201907L)
    ...
    [[nodiscard]] std::vector<tgui::Vector2f> drawRoundedRectHelperGetPoints(
        ...
        float offset)
    {
        ...
        for (unsigned int i = 0; i < nrCornerPoints; ++i)
        {
            points.emplace_back(offset + radius + (radius * std::cos(twoPi * ((2 * (nrCornerPoints - 1)) + i) / nrPointsInCircle)),
                                offset + size.y - radius
                                    - (radius * std::sin(twoPi * ((2 * (nrCornerPoints - 1)) + i) / nrPointsInCircle)));
        }

        // Bottom right corner
        for (unsigned int i = 0; i < nrCornerPoints; ++i)
        {
            points.emplace_back(offset + size.x - radius
                                    + (radius * std::cos(twoPi * ((3 * (nrCornerPoints - 1)) + i) / nrPointsInCircle)),
                                offset + size.y - radius
                                    - (radius * std::sin(twoPi * ((3 * (nrCornerPoints - 1)) + i) / nrPointsInCircle)));
        }
...
namespace tgui
{
    ...
    {
        TGUI_ASSERT(m_targetSize.x > 0 && m_targetSize.y > 0,
                    "Target size must be valid in BackendRenderTarget::addClippingLayer");
    ...
    void BackendRenderTarget::drawBorders(const RenderStates& states, const Borders& borders, Vector2f size, Color color)
    {
        ...
        const std::array<unsigned int, 8 * 3> indices = {
            {0, 2, 1, 1, 2, 3, 2, 4, 3, 3, 4, 5, 4, 6, 5, 5, 6, 7, 6, 1, 7, 7, 1, 8}};
