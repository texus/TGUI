...
namespace tgui
{
    ...
    bool BackendTextureGLES2::loadTextureOnly(Vector2u size, const std::uint8_t* pixels, bool smooth)
    {
        ...
        TGUI_ASSERT(getBackend()->getRenderer(),
                    "BackendTextureGLES2 can't be created when there is no backend renderer (was a gui attached to a window "
                    "yet?)");
        ...
        else
        {
            ...
            // GLES 2.0 does not support GL_REPEAT for NPOT textures, so we must use GL_CLAMP_TO_EDGE here
            TGUI_GL_CHECK(glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE));
            TGUI_GL_CHECK(glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE));
