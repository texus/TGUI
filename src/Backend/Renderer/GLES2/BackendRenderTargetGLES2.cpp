...
namespace tgui
{
    ...
    TGUI_NODISCARD static GLuint createShaderProgram()
    {
        ...
        if (vertexShader == 0)
            throw Exception{
                U"Failed to create shaders in BackendRenderTargetGLES2. glCreateShader(GL_VERTEX_SHADER) returned 0."};
        ...
        if (fragmentShader == 0)
            throw Exception{
                U"Failed to create shaders in BackendRenderTargetGLES2. glCreateShader(GL_FRAGMENT_SHADER) returned 0."};
        ...
        if (programLinked != GL_TRUE)
        {
            ...
            throw Exception{U"Failed to create shaders in BackendRenderTargetGLES2. Failed to link the shaders. Error: '"
                            + errorMessageStr + "'"};
        }
        ...
        m_shaderProgram(createShaderProgram())
    {
        TGUI_ASSERT(isBackendSet(),
                    "BackendRenderTargetGLES2 can't be created when there is no system backend initialized (was a gui created "
                    "yet?)");
    ...
    void BackendRenderTargetGLES2::setVertexAttribs()
    {
        ...
        TGUI_GL_CHECK(
            glVertexAttribPointer(m_positionShaderLocation, 2, GL_FLOAT, GL_FALSE, sizeof(Vertex), reinterpret_cast<GLvoid*>(0)));
        TGUI_GL_CHECK(
