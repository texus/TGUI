...
namespace tgui
{
    ...
    void BackendGuiSDL::mainLoop(Color clearColor)
    {
        TGUI_ASSERT(m_window && m_backendRenderTarget,
                    "BackendGuiSDL must be given an SDL_Window (either at construction or via setWindow function) "
    ...
    {
        TGUI_ASSERT(std::dynamic_pointer_cast<BackendSDL>(getBackend()),
                    "BackendGuiSDL requires system backend of type BackendSDL");
