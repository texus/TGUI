...
namespace tgui
{
    ...
    [[nodiscard]] static Event::KeyboardKey convertKeyCode(int key)
    {
        switch (key)
        {
            case GLFW_KEY_A:
    ...
    void BackendGuiGLFW::mainLoop(Color clearColor)
    {
        TGUI_ASSERT(m_window && m_backendRenderTarget,
                    "Gui must be given an GLFWwindow (either at construction or via setWindow function) before mainLoop() is "
                    "called");
        ...
        glfwSetMouseButtonCallback(m_window,
                                   ...
                                   {
                                       static_cast<BackendGuiGLFW*>(glfwGetWindowUserPointer(window))
    ...
    bool BackendGuiGLFW::isKeyboardModifierPressed(Event::KeyModifier modifierKey) const
    {
        switch (modifierKey)
        {
            case Event::KeyModifier::System:
                return (glfwGetKey(m_window, GLFW_KEY_LEFT_SUPER) == GLFW_PRESS)
    ...
    {
        TGUI_ASSERT(std::dynamic_pointer_cast<BackendGLFW>(getBackend()),
                    "BackendGuiGLFW requires system backend of type BackendGLFW");
