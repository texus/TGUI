...
namespace tgui
{
    ...
    bool BackendFontFreetype::loadFromMemory(const void* data, std::size_t sizeInBytes)
    {
        cleanup();
        ...
        FT_Face face;
        if (FT_New_Memory_Face(library, static_cast<const FT_Byte*>(data), static_cast<FT_Long>(sizeInBytes), 0, &face) != 0)
        {
    ...
    #pragma GCC diagnostic pop
        ...
        glyph.lsbDelta = m_face->glyph->lsb_delta;
        glyph.rsbDelta = m_face->glyph->rsb_delta;
