...
namespace tgui
{
    ...
    bool BackendFontSDLttf::loadFromMemory(const void* data, std::size_t sizeInBytes)
    {
        m_glyphs.clear();
        m_rows.clear();
        m_pixels = nullptr;
        ...
        m_fileContents = MakeUniqueForOverwrite<std::uint8_t[]>(sizeInBytes);
    ...
    FontGlyph BackendFontSDLttf::getGlyph(char32_t codePoint, unsigned int characterSize, bool bold, float outlineThickness)
    {
        ...
        if (surface)
        {
            ...
            // Some pixels around the glyph are empty. The bounds of the glyphs specify which part is filled,
            // but it seems like we don't know the top as we don't know where the baseline is?
            int top = 0;
            bool topFound = false;
            if ((glyph.bounds.left >= 0) && (glyph.bounds.left + glyph.bounds.width <= surface->w))
            {
                for (int y = 0; !topFound && (y < surface->h); ++y)
                {
                    for (int x = glyph.bounds.left; x < glyph.bounds.left + glyph.bounds.width; ++x)
                    {
                        if (surfacePixels[(y * surface->pitch) + x] == 0)
                            continue;
            ...
            if (topFound && (top + glyph.bounds.height <= surface->h))
            {
                ...
                glyph.textureRect = findAvailableGlyphRect(glyph.bounds.width + (2 * padding), glyph.bounds.height + (2 * padding));
                ...
                for (int y = top; y < top + glyph.bounds.height; ++y)
                {
                    for (int x = glyph.bounds.left; x < glyph.bounds.left + glyph.bounds.width; ++x)
                    {
                        const std::size_t index = (glyph.textureRect.left + x - glyph.bounds.left) + (glyph.textureRect.top + y - top) * m_textureSize;
    ...
    {
        return getUnderlineInfo(characterSize).first;
    }
    ...
    {
        return getUnderlineInfo(characterSize).second;
    }
    ...
    std::pair<int, int> BackendFontSDLttf::getUnderlineInfo(unsigned int characterSize)
    {
        ...
        if (surface)
        {
            /// TODO: Retest this code. y started one higher originally, which should have been an out of bound situation
