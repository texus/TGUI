...
namespace tgui
{
...
#elif (SDL_TTF_MAJOR_VERSION == 2 && SDL_TTF_MINOR_VERSION > 0) \
    || (SDL_TTF_MAJOR_VERSION == 2 && SDL_TTF_MINOR_VERSION == 0 && SDL_TTF_PATCHLEVEL >= 18)
    ...
    || (SDL_TTF_MAJOR_VERSION == 2 && SDL_TTF_MINOR_VERSION == 0 && SDL_TTF_PATCHLEVEL >= 18)
        ...
        if (surface)
        {
            ...
            const unsigned int surfaceTop = static_cast<unsigned int>(
                std::max(0, TTF_GetFontAscent(font) - maxY)); // assumed to be always positive, even without max
            ...
            if ((surface->w > 0) && (surface->h > 0) && (surfaceLeft + surfaceWidth <= static_cast<unsigned int>(surface->w))
                && (surfaceTop + surfaceHeight <= static_cast<unsigned int>(surface->h)))
            {
                ...
                for (unsigned int y = surfaceTop; y < surfaceTop + surfaceHeight; ++y)
                {
                    ...
                    {
                        const std::size_t index = (glyph.textureRect.left + x - surfaceLeft)
                                                  + (glyph.textureRect.top + y - surfaceTop) * m_textureSize;
...
#if TGUI_USE_SDL3
        SDL_IOStream* handle = SDL_IOFromConstMem(m_fileContents.get(), m_fileSize);
        if (!handle)
            return nullptr;
    ...
    || (SDL_TTF_MAJOR_VERSION == 2 && SDL_TTF_MINOR_VERSION == 0 && SDL_TTF_PATCHLEVEL >= 18)
        ...
        if (surface)
        {
            ...
            {
                const bool isUnderlinePixel = (static_cast<const std::uint8_t*>(surface->pixels)[(y * surface->pitch) + (surface->w / 2)]
                                               != 0);
