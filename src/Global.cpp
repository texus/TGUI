...
namespace tgui
{
    ...
    std::unique_ptr<std::uint8_t[]> readFileToMemory(const String& filename, std::size_t& fileSize)
    {
...
#endif
        {
            ...
            {
                if (fclose(fp))
                    throw tgui::Exception{U"Failed to fclose file, error: "
                                          + tgui::String(std::system_error(errno, std::generic_category()).what())};
            };
    ...
    bool writeFile(const String& filename, CharStringView textToWrite)
    {
        ...
        if (fclose(file))
            throw tgui::Exception{U"writeFile failed to fclose file, error: "
                                  + tgui::String(std::system_error(errno, std::generic_category()).what())};
