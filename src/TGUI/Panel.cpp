...
namespace tgui
{
    ...
    bool Panel::setBackgroundImage(const std::string filename)
    {
        ...
        if (TGUI_TextureManager.getTexture(filename, m_Texture))
        {
            ...
            // Set the size of the sprite
            m_Sprite.setScale(static_cast<float>(m_RenderTexture->getSize().x) / m_Texture->getSize().x, static_cast<float>(m_RenderTexture->getSize().y) / m_Texture->getSize().y);
    ...
    void Panel::draw(sf::RenderTarget& target, sf::RenderStates states) const
    {
        ...
        drawObjectGroup(m_RenderTexture);
