...
namespace tgui
{
    ...
    void Label::draw(sf::RenderTarget& target, sf::RenderStates states) const
    {
        ...
        sf::Vector2f bottomRightPosition
            = states.transform.transformPoint((getPosition().x + m_background.getSize().x - view.getCenter().x + (view.getSize().x / 2.f)) * view.getViewport().width + (view.getSize().x * view.getViewport().left),
                                              (getPosition().y + m_background.getSize().y - view.getCenter().y + (view.getSize().y / 2.f)) * view.getViewport().height + (view.getSize().y * view.getViewport().top));
