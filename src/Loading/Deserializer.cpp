        ...
        ObjectConverter deserializeColor(const String& value)
        {
            ...
            if (str[0] == '#')
            {
                if ((str.length() != 4) && (str.length() != 5) && (str.length() != 7) && (str.length() != 9))
                    throw Exception{
                        U"Failed to deserialize color '" + value + U"'. Value started but '#' but has the wrong length."};
                ...
                while (removeWhitespace(value, c))
                {
                    ...
                    if (closeBracketPos != String::npos)
                    {
                        if (!readUIntRect(value.substr(endOffset, closeBracketPos - endOffset + 1), *rect, rectRequiresFourValues))
                            throw Exception{
                                U"Failed to parse " + word + U" rectangle while deserializing texture '" + value + U"'."};
                    }
                    else
                        throw Exception{U"Failed to deserialize texture '" + value + U"'. Failed to find closing bracket for "
                                        + word + U" rectangle."};
