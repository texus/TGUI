...
namespace tgui
{
    ...
    [[nodiscard]] static std::time_t FileTimeToUnixTime(FILETIME const& FileTime)
    {
    ...
    std::vector<Filesystem::FileInfo> Filesystem::listFilesInDirectory(const Path& path)
    {
        ...
        do
        {
            ...
            if (!fileInfo.directory)
                fileInfo.fileSize = (entry.nFileSizeHigh * (static_cast<decltype(fileInfo.fileSize)>(MAXDWORD) + 1))
                                    + entry.nFileSizeLow;
