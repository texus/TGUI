...
namespace tgui
{
    ...
    namespace
    {
        ...
        void makePathsRelativeToForm(const std::unique_ptr<DataIO::Node>& node, const String& formPath)
        {
            ...
            {
                if (!pair.first.starts_with(U"Texture") && (pair.first != U"Font") && (pair.first != U"Image")
                    && (pair.first != U"Icon"))
                    continue;
    ...
    void Container::removeAllWidgets()
    {
        ...
        // If a widget still has focus then we will trigger its unfocus event now.
    ...
    void Container::load(const std::unique_ptr<DataIO::Node>& node, const LoadingRenderersMap& renderers)
    {
        ...
        for (const auto& childNode : node->children)
        {
            ...
            if (constructor)
            {
                String className;
                if (nameSeparator != String::npos)
                    className = Deserializer::deserialize(ObjectConverter::Type::String, childNode->name.substr(nameSeparator + 1))
                                    .getString();
    ...
    void Container::injectFormFilePath(const std::unique_ptr<DataIO::Node>& node,
                                       const String& path,
                                       std::map<String, bool>& checkedFilenames) const
    {
        for (const auto& pair : node->propertyValuePairs)
        {
                ...
                || (pair.first == U"Image") || (pair.first == U"Icon"))
            {
                ...
                else
                {
                    ...
                    TGUI_ASSERT(endQuotePos != String::npos,
                                "End quote must exist in Container::injectFormFilePath, DataIO could not accept the value "
                                "otherwise");
                    filename = pair.second->value.substr(1, endQuotePos - 1);
                }
    ...
    constexpr char RootContainer::StaticWidgetType[];
#endif
