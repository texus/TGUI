...
namespace tgui
{
    ...
    constexpr char RadioButtonGroup::StaticWidgetType[];
#endif
    ...
    bool RadioButtonGroup::isMouseOnWidget(Vector2f pos) const
    {
        ...
        return std::any_of(m_widgets.cbegin(),
                           m_widgets.cend(),
                           [&](const auto& widget)
                           { return widget->isVisible() && widget->isMouseOnWidget(transformMousePos(widget, pos - offset)); });
    }
