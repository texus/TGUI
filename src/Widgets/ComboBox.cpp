...
namespace tgui
{
    ...
    constexpr char ComboBox::StaticWidgetType[];
#endif
        ...
        Widget{typeName, false}
    {
        ...
        if (initRenderer)
        {
            ...
            setSize({m_text.getLineHeight() * 10,
                     std::round(m_text.getLineHeight() * 1.25f) + m_paddingCached.getTopPlusBottom()
    ...
    void ComboBox::setSize(const Layout2d& size)
    {
        ...
        {
            m_spriteArrow.setSize(
                {m_spriteArrow.getTexture().getImageSize().x * (height / m_spriteArrow.getTexture().getImageSize().y), height});
    ...
    void ComboBox::addMultipleItems(const std::vector<String>& itemNames)
    {
        m_listBox->addMultipleItems(itemNames);
        updateListBoxHeight();
    }
    ...
    String ComboBox::getItemByIndex(std::size_t index) const
    {
        return m_listBox->getItemByIndex(index);
    }

    ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

    int ComboBox::getIndexById(const String& id) const
    {
        return m_listBox->getIndexById(id);
    }

    ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

    String ComboBox::getIdByIndex(std::size_t index) const
    {
        return m_listBox->getIdByIndex(index);
    }
    ...
    void ComboBox::setItemData(std::size_t index, Any data)
    {
        m_listBox->setItemData(index, std::move(data));
    }
    ...
    bool ComboBox::scrolled(float delta, Vector2f, bool touch)
    {
        ...
        if (delta < 0)
        {
            ...
            const std::size_t nextIndex = (m_listBox->getSelectedItemIndex() < 0)
                                              ? 0
                                              : static_cast<std::size_t>(m_listBox->getSelectedItemIndex()) + 1;
    ...
    std::unique_ptr<DataIO::Node> ComboBox::save(SavingRenderersMap& renderers) const
    {
        ...
        node->propertyValuePairs[U"ChangeItemOnScroll"] = std::make_unique<DataIO::ValueNode>(
            Serializer::serialize(m_changeItemOnScroll));
        ...
        if (m_listBox->getSelectedItemIndex() >= 0)
            node->propertyValuePairs[U"SelectedItemIndex"] = std::make_unique<DataIO::ValueNode>(
                String::fromNumber(m_listBox->getSelectedItemIndex()));
    ...
    void ComboBox::load(const std::unique_ptr<DataIO::Node>& node, const LoadingRenderersMap& renderers)
    {
        ...
        if (node->propertyValuePairs[U"Items"])
        {
            ...
            if (node->propertyValuePairs[U"ItemIds"])
            {
                ...
                {
                    addItem(Deserializer::deserialize(ObjectConverter::Type::String, node->propertyValuePairs[U"Items"]->valueList[i])
                                .getString(),
        ...
        if (node->propertyValuePairs[U"ChangeItemOnScroll"])
            m_changeItemOnScroll = Deserializer::deserialize(ObjectConverter::Type::Bool,
                                                             node->propertyValuePairs[U"ChangeItemOnScroll"]->value)
                                       .getBool();
    ...
    void ComboBox::showListBox()
    {
        ...
        {
            if ((getAbsolutePosition().y + getSize().y + m_listBox->getSize().y - m_bordersCached.getBottom()
                 > container->getSize().y)
    ...
    void ComboBox::draw(BackendRenderTarget& target, RenderStates states) const
    {
        ...
        else // There are no textures for the arrow
        {
            ...
            if ((m_roundedBorderRadiusCached > 0) && (m_roundedBorderRadiusCached > m_bordersCached.getTop()))
            {
                ...
                target.drawRoundedRectangle(states,
                                            {arrowSize, arrowSize},
                                            Color::applyOpacity(arrowBackgroundColor, m_opacityCached),
                                            roundedRadius,
                                            {},
                                            {});
            }
        ...
        if (((selectedItemIndex >= 0) && !m_text.getString().empty())
            || ((selectedItemIndex < 0) && !m_defaultText.getString().empty()))
        {
            target.addClippingLayer(statesForText,
                                    {{m_paddingCached.getLeft(), m_paddingCached.getTop()},
                                     {getInnerSize().x - m_paddingCached.getLeftPlusRight() - arrowSize,
                                      getInnerSize().y - m_paddingCached.getTopPlusBottom()}});

            statesForText.transform.translate(
                {m_paddingCached.getLeft() + m_text.getExtraHorizontalPadding(),
