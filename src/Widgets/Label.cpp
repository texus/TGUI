...
namespace tgui
{
    ...
    constexpr char Label::StaticWidgetType[];
#endif
    ...
    std::unique_ptr<DataIO::Node> Label::save(SavingRenderersMap& renderers) const
    {
        ...
        if (m_maximumTextWidth > 0)
            node->propertyValuePairs[U"MaximumTextWidth"] = std::make_unique<DataIO::ValueNode>(
                String::fromNumber(m_maximumTextWidth));
        if (m_autoSize)
    ...
    void Label::rearrangeText()
    {
        ...
        float requiredTextHeight = ((lineCount - 1) * m_fontCached.getLineSpacing(m_textSizeCached))
                                   + std::max(m_fontCached.getFontHeight(m_textSizeCached),
                                              m_fontCached.getLineSpacing(m_textSizeCached))
