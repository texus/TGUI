...
namespace tgui
{
    ...
    void EditBox::load(const std::unique_ptr<DataIO::Node>& node, const LoadingRenderersMap& renderers)
    {
        ...
        if (node->propertyValuePairs[U"Suffix"])
            setSuffix(
                Deserializer::deserialize(ObjectConverter::Type::String, node->propertyValuePairs[U"Suffix"]->value).getString());
        if (node->propertyValuePairs[U"PasswordCharacter"])
        {
            const String pass = Deserializer::deserialize(ObjectConverter::Type::String,
                                                          node->propertyValuePairs[U"PasswordCharacter"]->value)
                                    .getString();
            if (!pass.empty())
        ...
        if (node->propertyValuePairs[U"InputValidator"])
        {
            if (node->propertyValuePairs[U"InputValidator"]->value == U"Int")
                ...
                setInputValidator(
                    Deserializer::deserialize(ObjectConverter::Type::String, node->propertyValuePairs[U"InputValidator"]->value)
                        .getString());
        }
    }
    ...
    void EditBox::recalculateTextPositions()
    {
        ...
        const float textY = m_paddingCached.getTop()
                            + (((getInnerSize().y - m_paddingCached.getBottom() - m_paddingCached.getTop()) - m_textFull.getSize().y)
                               / 2.f);
