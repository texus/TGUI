...
namespace tgui
{
    ...
    Vector2f RadioButton::getFullSize() const
    {
        if (getText().empty())
            return getSize();
        return {getSize().x + (getSize().x * m_textDistanceRatioCached) + m_text.getSize().x,
                std::max(getSize().y, m_text.getSize().y)};
    }
    ...
    void RadioButton::load(const std::unique_ptr<DataIO::Node>& node, const LoadingRenderersMap& renderers)
    {
        ...
        if (node->propertyValuePairs[U"MaxWidth"])
            setMaxWidth(
                Deserializer::deserialize(ObjectConverter::Type::Number, node->propertyValuePairs[U"MaxWidth"]->value).getNumber());
    }
