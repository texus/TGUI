...
namespace tgui
{
    ...
    void ChildWindow::mouseMoved(Vector2f pos)
    {
        ...
        else // Not dragging child window
        {
            ...
            else
            {
                ...
                // Check if the mouse is on top of the title bar
                if (FloatRect{m_bordersCached.getLeft(), m_bordersCached.getTop(), getInnerTitleBarSize().x, m_titleBarHeightCached}
                        .contains(pos))
                {
    ...
    void ChildWindow::rendererChanged(const String& property)
    {
        ...
        else if (property == U"CloseButton")
        {
            ...
            if (!maximizeButtonRenderer
                || (maximizeButtonRenderer->propertyValuePairs.empty() && !maximizeButtonRenderer->connectedTheme))
                m_maximizeButton->setRenderer(closeButtonRenderer);

            const auto minimizeButtonRenderer = getSharedRenderer()->getMinimizeButton();
            if (!minimizeButtonRenderer
                || (minimizeButtonRenderer->propertyValuePairs.empty() && !minimizeButtonRenderer->connectedTheme))
    ...
    void ChildWindow::load(const std::unique_ptr<DataIO::Node>& node, const LoadingRenderersMap& renderers)
    {
        ...
        if (node->propertyValuePairs[U"Resizable"])
            setResizable(
                Deserializer::deserialize(ObjectConverter::Type::Bool, node->propertyValuePairs[U"Resizable"]->value).getBool());
