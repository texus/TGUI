...
namespace tgui
{
    ...
    constexpr char Slider::StaticWidgetType[];
#endif
    ...
    void Slider::mouseMoved(Vector2f pos)
    {
        ...
        if (m_orientation == Orientation::Vertical)
        {
            ...
            float value = m_thumbWithinTrackCached
                              ? m_maximum
                                    - (((pos.y - m_mouseDownOnThumbPos.y) / (getSize().y - m_thumb.height)) * (m_maximum - m_minimum))
                              : m_maximum
    ...
    std::unique_ptr<DataIO::Node> Slider::save(SavingRenderersMap& renderers) const
    {
        ...
        node->propertyValuePairs[U"InvertedDirection"] = std::make_unique<DataIO::ValueNode>(
            Serializer::serialize(m_invertedDirection));
    ...
    void Slider::draw(BackendRenderTarget& target, RenderStates states) const
    {
        ...
        else // There are no textures
        {
            ...
            const Vector2f thumbInnerSize = {m_thumb.width - m_bordersCached.getLeftPlusRight(),
                                             m_thumb.height - m_bordersCached.getTopPlusBottom()};
