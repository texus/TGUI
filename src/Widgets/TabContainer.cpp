...
namespace tgui
{
    ...
    constexpr char TabContainer::StaticWidgetType[];
#endif
    ...
    void TabContainer::load(const std::unique_ptr<DataIO::Node>& node, const LoadingRenderersMap& renderers)
    {
        ...
        if (node->propertyValuePairs[U"TabAlignment"])
        {
            const String alignment = Deserializer::deserialize(ObjectConverter::Type::String,
                                                               node->propertyValuePairs[U"TabAlignment"]->value)
                                         .getString();
        ...
        if (node->propertyValuePairs[U"TabFixedSize"])
            tabFixedSize = Deserializer::deserialize(ObjectConverter::Type::Number, node->propertyValuePairs[U"TabFixedSize"]->value)
                               .getNumber();
