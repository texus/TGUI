...
namespace tgui
{
    ...
    void FileDialog::changePath(const Filesystem::Path& path, bool updateHistory)
    {
        if (updateHistory && (m_currentDirectory != path))
        {
            if (m_pathHistoryIndex + 1 < m_pathHistory.size())
                m_pathHistory.erase(m_pathHistory.cbegin() + static_cast<std::ptrdiff_t>(m_pathHistoryIndex + 1),
                                    m_pathHistory.cend());
            ...
            if (!file.directory && !m_fileTypeFilters[m_selectedFileTypeFilter].second.empty())
            {
                ...
                for (const auto& filter : m_fileTypeFilters[m_selectedFileTypeFilter].second)
                {
                    TGUI_ASSERT(!filter.empty(),
                                "FileDialog::sortFilesInListView can't have empty filter, they are removed in "
                                "setFileTypeFilters");
    ...
    && defined(__GNUC__) // MinGW doesn't support %e (day of the month without leading 0) and %R (same as %H:%M)
            ...
            if ((filename.length() > 4)
                && ((filename.compare(filenameLen - 4, 4, U".lnk", 4) == 0)
                    || (filename.compare(filenameLen - 4, 4, U".url", 4) == 0)))
    ...
    std::unique_ptr<DataIO::Node> FileDialog::save(SavingRenderersMap& renderers) const
    {
        ...
        node->propertyValuePairs[U"SelectingDirectory"] = std::make_unique<DataIO::ValueNode>(
            Serializer::serialize(m_selectingDirectory));
        ...
        if (!m_fileTypeFilters.empty())
        {
            for (const auto& filter : m_fileTypeFilters)
            {
                ...
                filterNode->propertyValuePairs[U"Description"] = std::make_unique<DataIO::ValueNode>(
                    Serializer::serialize(filter.first));
    ...
    void FileDialog::load(const std::unique_ptr<DataIO::Node>& node, const LoadingRenderersMap& renderers)
    {
        ...
        for (const auto& childNode : node->children)
        {
            ...
            if (!childNode->propertyValuePairs[U"Pattern"]->listNode)
                throw Exception{
                    U"Failed to parse 'Pattern' property inside the 'FileTypeFilter' property, expected a list as value"};
