...
namespace tgui
{
    ...
    void MenuWidgetBase::loadMenus(const std::unique_ptr<DataIO::Node>& node, std::vector<Menu>& menus)
    {
        for (const auto& childNode : node->children)
        {
            ...
            if (childNode->propertyValuePairs[U"Enabled"])
                menus.back().enabled = Deserializer::deserialize(ObjectConverter::Type::Bool,
                                                                 childNode->propertyValuePairs[U"Enabled"]->value)
                                           .getBool();
    ...
    Vector2f MenuWidgetBase::calculateSubmenuOffset(
        const Menu& menu,
        float globalLeftPos,
        float menuWidth,
        float subMenuWidth,
