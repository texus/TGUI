...
namespace tgui
{
    ...
    void RangeSlider::mouseMoved(Vector2f pos)
    {
        ...
        if (m_mouseDown && (m_mouseDownOnThumb != 0))
        {
            ...
            if (m_orientation == Orientation::Vertical)
            {
                if (m_mouseDownOnThumb == 1)
                {
                    setSelectionStart(m_maximum
                                      - (((pos.y + (m_thumbs.first.height / 2.0f) - m_mouseDownOnThumbPos.y) / getSize().y)
                                         * (m_maximum - m_minimum)));
                ...
                else // if (m_mouseDownOnThumb == 2)
                {
                    ...
                    // Set the thumb position for smooth scrolling
                    const float thumbTop = pos.y - m_mouseDownOnThumbPos.y;
                    if ((thumbTop + (m_thumbs.second.height / 2.0f) > 0)
                        && (thumbTop + (m_thumbs.second.height / 2.0f) < getSize().y))
            ...
            else // the slider lies horizontal
            {
                ...
                {
                    setSelectionEnd((((pos.x + (m_thumbs.second.width / 2.0f) - m_mouseDownOnThumbPos.x) / getSize().x)
                                     * (m_maximum - m_minimum))
                                    + m_minimum);

                    // Set the thumb position for smooth scrolling
                    const float thumbLeft = pos.x - m_mouseDownOnThumbPos.x;
                    if ((thumbLeft + (m_thumbs.second.width / 2.0f) > 0)
                        && (thumbLeft + (m_thumbs.second.width / 2.0f) < getSize().x))
    ...
    void RangeSlider::updateThumbPositions()
    {
        ...
        if (m_orientation == Orientation::Vertical)
        {
            ...
            m_thumbs.first.top = (innerSize.y / (m_maximum - m_minimum) * (m_maximum - m_selectionStart))
                                 - (m_thumbs.first.height / 2.0f);

            m_thumbs.second.left = m_bordersCached.getLeft() + ((innerSize.x - m_thumbs.second.width) / 2.0f);
            m_thumbs.second.top = (innerSize.y / (m_maximum - m_minimum) * (m_maximum - m_selectionEnd))
                                  - (m_thumbs.second.height / 2.0f);
        }
        else // horizontal
        {
            m_thumbs.first.left = (innerSize.x / (m_maximum - m_minimum) * (m_selectionStart - m_minimum))
                                  - (m_thumbs.first.width / 2.0f);
            m_thumbs.first.top = m_bordersCached.getTop() + ((innerSize.y - m_thumbs.first.height) / 2.0f);

            m_thumbs.second.left = (innerSize.x / (m_maximum - m_minimum) * (m_selectionEnd - m_minimum))
                                   - (m_thumbs.second.width / 2.0f);
            ...
            if (m_imageOrientation == m_orientation)
            {
                if (m_orientation == Orientation::Vertical)
                    m_spriteSelectedTrack.setVisibleRect(
                        {0,
                         m_thumbs.second.top + (m_thumbs.first.height / 2.0f),
                         innerSize.x,
                         m_thumbs.first.top - m_thumbs.second.top});
                else
                    m_spriteSelectedTrack.setVisibleRect(
    ...
    void RangeSlider::draw(BackendRenderTarget& target, RenderStates states) const
    {
        ...
        // Draw the first thumb
        {
            states.transform.translate(
                {-m_bordersCached.getLeft() + m_thumbs.first.left, -m_bordersCached.getTop() + m_thumbs.first.top});
