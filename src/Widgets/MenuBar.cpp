...
namespace tgui
{
    ...
    constexpr char MenuBar::StaticWidgetType[];
#endif
    ...
    void MenuBar::mouseMoved(Vector2f pos)
    {
        ...
        for (std::size_t i = 0; i < m_menus.size(); ++i)
        {
            ...
            if (visibleMenuIdx == i)
            {
                ...
                {
                    updateMenuTextColor(m_menus[i].menuItems[static_cast<std::size_t>(m_menus[visibleMenuIdx].selectedMenuItem)],
                                        false);
    ...
    void MenuBar::load(const std::unique_ptr<DataIO::Node>& node, const LoadingRenderersMap& renderers)
    {
        ...
        if (node->propertyValuePairs[U"InvertedMenuDirection"])
            setInvertedMenuDirection(
                Deserializer::deserialize(ObjectConverter::Type::Bool, node->propertyValuePairs[U"InvertedMenuDirection"]->value)
                    .getBool());
