...
namespace tgui
{
    namespace
    {
        ...
        void saveItems(const std::unique_ptr<DataIO::Node>& parentNode, const std::vector<std::shared_ptr<TreeView::Node>>& items)
        {
            for (const auto& item : items)
            {
                ...
                itemNode->propertyValuePairs[U"Text"] = std::make_unique<DataIO::ValueNode>(
                    Serializer::serialize(item->text.getString()));
        ...
        Widget{typeName, false}
    {
        if (initRenderer)
        {
            m_renderer = makeCopied<TreeViewRenderer>();
        ...
        m_textStyleCached{other.m_textStyleCached}
    {
        ...
        TGUI_ASSERT(std::count(m_visibleNodes.cbegin(), m_visibleNodes.cend(), nullptr) == 0,
                    "Cloned nodes can't contain a nullptr");
    }
    ...
    {
        return downcast<TreeViewRenderer*>(Widget::getSharedRenderer());
    }
    ...
    {
        return downcast<const TreeViewRenderer*>(Widget::getSharedRenderer());
    }
    ...
    {
        return downcast<TreeViewRenderer*>(Widget::getRenderer());
    }
    ...
    unsigned int TreeView::getVerticalScrollbarMaxValue() const
    {
    ...
    unsigned int TreeView::getHorizontalScrollbarMaxValue() const
    {
        ...
        if (m_mouseDown && !m_verticalScrollbar->isMouseDown() && !m_horizontalScrollbar->isMouseDown())
        {
                    ...
                    .contains(pos))
            {
                ...
                if ((selectedIndex >= 0) && (selectedIndex == m_selectedItem))
                {
                    ...
                    if (FloatRect{iconOffsetX + m_bordersCached.getLeft() + m_paddingCached.getLeft()
                                      - m_horizontalScrollbar->getValue(),
                                  iconOffsetY + (static_cast<unsigned int>(selectedIndex) * m_itemHeight)
                                      + m_bordersCached.getTop() + m_paddingCached.getTop() - m_verticalScrollbar->getValue(),
                                  m_iconBounds.x,
                                  m_iconBounds.y}
                            .contains(pos))
                    {
    ...
    void TreeView::draw(BackendRenderTarget& target, RenderStates states) const
    {
        ...
        states.transform.translate({m_paddingCached.getLeft() - m_horizontalScrollbar->getValue(),
            ...
            else // Branch node
            {
                ...
                else // No textures are used
                {
                    ...
                    if (m_visibleNodes[i]->expanded)
                    {
                        ...
                        target.drawFilledRect(statesForIcon,
                                              {m_iconBounds.x, thickness},
                                              Color::applyOpacity(iconColor, m_opacityCached));
                    }
                    else // Collapsed node
                    {
                        // Draw "+"
