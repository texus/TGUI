...
namespace tgui
{
    ...
    {
        return std::find_if(m_items.cbegin(),
                            m_items.cend(),
                            [itemStr](const Item& item) { return item.text.getString() == itemStr; })
               != m_items.cend();
    }
    ...
    void ListBox::setHoveredItemBasedOnMousePos(Vector2f innerPos)
    {
        ...
        const int hoveringItem = static_cast<int>(
            ((innerPos.y - (m_itemHeight - (m_scrollbar->getValue() % m_itemHeight))) / m_itemHeight)
