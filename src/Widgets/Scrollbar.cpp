...
namespace tgui
{
    ...
    constexpr char Scrollbar::StaticWidgetType[];
#endif
                        ...
                        if (pos.y < getSize().y - m_arrowUp.height)
                        {
                            ...
                            if (value <= m_value)
                            {
                                ...
                                if (value >= subtractValue)
                                    setValue(static_cast<unsigned int>(std::lround(value - subtractValue)));
                                else
                                    setValue(0);
                            }
    ...
    bool Scrollbar::scrolled(float delta, Vector2f pos, bool touch)
    {
        ...
        const bool scrollingIsPossible = ((oldValue != 0) || (delta <= 0))
                                         && ((oldValue != m_maximum - m_viewportSize) || (delta >= 0));
    ...
    void Scrollbar::load(const std::unique_ptr<DataIO::Node>& node, const LoadingRenderersMap& renderers)
    {
        ...
        {
            const bool autoHide = Deserializer::deserialize(ObjectConverter::Type::Bool, node->propertyValuePairs[U"AutoHide"]->value)
                                      .getBool();
            setPolicy(autoHide ? Policy::Automatic : Policy::Always);
        }
#endif
    ...
    void Scrollbar::callMousePressPeriodically(std::chrono::time_point<std::chrono::steady_clock> clickedTime, bool repeatedCall)
    {
        ...
        Timer::scheduleCallback(
            [widgetPtr, clickedTime]()
            {
                ...
                if (scrollbar->m_mouseHover && scrollbar->m_mouseDownOnDecreaseArrow
                    && (scrollbar->m_mouseHoverOverPart == Part::ArrowUp))
                {
                         ...
                         && (scrollbar->m_mouseHoverOverPart == Part::ArrowDown))
                {
                    ...
                    {
                        if (scrollbar->m_value % scrollbar->m_scrollAmount)
                            scrollbar->setValue(scrollbar->m_value
                                                + (scrollbar->m_scrollAmount - (scrollbar->m_value % scrollbar->m_scrollAmount)));
                        else
                            scrollbar->setValue(scrollbar->m_value + scrollbar->m_scrollAmount);
                    }
                    else
    ...
    void Scrollbar::draw(BackendRenderTarget& target, RenderStates states) const
    {
        ...
        else
        {
            if (m_mouseHover && (m_mouseHoverOverPart == Scrollbar::Part::Track) && m_trackColorHoverCached.isSet())
                target.drawFilledRect(states,
                                      {m_track.width, m_track.height},
                                      Color::applyOpacity(m_trackColorHoverCached, m_opacityCached));
            else
