    ...
    void Knob::setSize(const Layout2d& size)
    {
        ...
        if (m_spriteForeground.isSet())
        {
            m_spriteForeground.setSize(
                {(m_spriteForeground.getTexture().getImageSize().x
                  / static_cast<float>(m_spriteBackground.getTexture().getImageSize().x))
                     * getSize().x,
                 (m_spriteForeground.getTexture().getImageSize().y
                  / static_cast<float>(m_spriteBackground.getTexture().getImageSize().y))
                     * getSize().y});
        }
    }
    ...
    void Knob::draw(BackendRenderTarget& target, RenderStates states) const
    {
        ...
        else
        {
            ...
            states.transform.translate(
                {(innerSize / 2.0f) - radius + (std::cos(m_angle / 180 * pi) * (innerSize / 2.f) * 3.f / 5.f),
