...
namespace tgui
{
    ...
    void BitmapButton::load(const std::unique_ptr<DataIO::Node>& node, const LoadingRenderersMap& renderers)
    {
        ...
        if (node->propertyValuePairs[U"Image"])
            setImage(
                Deserializer::deserialize(ObjectConverter::Type::Texture, node->propertyValuePairs[U"Image"]->value).getTexture());
    ...
    void BitmapButton::updateSize()
    {
        ...
        if (m_autoSize)
        {
            ...
            Widget::setSize(
                {getSize().x, std::round(m_textComponent->getLineHeight() * 1.25f) + borders.getTop() + borders.getBottom()});
    ...
    void BitmapButton::updateTextPosition()
    {
        ...
        m_imageComponent->setPosition({m_textPosition.x.getValue() - (m_textOrigin.x * contentSize.x),
                                       m_textPosition.y.getValue() - (m_textOrigin.y * contentSize.y)
                                           + ((contentSize.y - m_imageComponent->getSize().y) / 2.f)});
