...
namespace tgui
{
    static constexpr std::array<std::pair<StringView, char32_t>, 4> symbolNamesMap{
        {{U"lt"sv, U'<'}, {U"gt"sv, U'>'}, {U"amp"sv, U'&'}}};
    ...
    void RichTextLabel::rearrangeText()
    {
        ...
        Optional<std::vector<std::vector<Text::Blueprint>>>
            wordWrappedLines = (maxWidth > 0) ? Text::wordWrap(maxWidth, textPiecesLines, m_fontCached)
                                              : Optional<std::vector<std::vector<Text::Blueprint>>>();
        ...
        for (const auto& textPiecesLine : *textPiecesLinesPtr)
        {
            ...
            for (std::size_t j = 0; j < textPiecesLine.size(); ++j)
            {
                ...
                else // Normal piece of text instead of an image
                {
                    ...
                    if (j > 0 && !textPiecesLine[j - 1].text.empty() && !textPiecesLine[j].text.empty())
                    {
                        ...
                        const unsigned int characterSize = std::min(textPiecesLine[j - 1].characterSize,
                                                                    textPiecesLine[j].characterSize);
            ...
            for (std::size_t j = imageIndexBeforeLine; j < imageIndex; ++j)
            {
                ...
                if (pieceHeight != maxLineHeight)
                    m_images[j].setPosition(
                        {m_images[j].getPosition().x, m_images[j].getPosition().y + (maxLineHeight - pieceHeight)});
            }
