...
namespace tgui
{
    ...
    constexpr char ChatBox::StaticWidgetType[];
#endif
    ...
    std::size_t ChatBox::getLineAmount() const
    {
        return m_lines.size();
    }
    ...
    std::size_t ChatBox::getLineLimit() const
    {
        return m_maxLines;
    }
    ...
    void ChatBox::recalculateFullTextHeight()
    {
        ...
        if (m_newLinesBelowOthers)
        {
            if (((oldMaximum >= m_scrollbar->getViewportSize())
                 && (m_scrollbar->getValue() == oldMaximum - m_scrollbar->getViewportSize()))
    ...
    void ChatBox::load(const std::unique_ptr<DataIO::Node>& node, const LoadingRenderersMap& renderers)
    {
        ...
        if (node->propertyValuePairs[U"TextColor"])
            setTextColor(
                Deserializer::deserialize(ObjectConverter::Type::Color, node->propertyValuePairs[U"TextColor"]->value).getColor());
        if (node->propertyValuePairs[U"TextStyle"])
            setTextStyle(Deserializer::deserialize(ObjectConverter::Type::TextStyle, node->propertyValuePairs[U"TextStyle"]->value)
                             .getTextStyle());
        ...
        for (const auto& childNode : node->children)
        {
            if (childNode->name == U"Line")
            {
                ...
                if (childNode->propertyValuePairs[U"Color"])
                    lineTextColor = Deserializer::deserialize(ObjectConverter::Type::Color,
                                                              childNode->propertyValuePairs[U"Color"]->value)
                                        .getColor();

                TextStyles lineTextStyle = getTextStyle();
                if (childNode->propertyValuePairs[U"Style"])
                    lineTextStyle = Deserializer::deserialize(ObjectConverter::Type::TextStyle,
                                                              childNode->propertyValuePairs[U"Style"]->value)
                                        .getTextStyle();

                if (childNode->propertyValuePairs[U"Text"])
                    addLine(Deserializer::deserialize(ObjectConverter::Type::String, childNode->propertyValuePairs[U"Text"]->value)
                                .getString(),
                            lineTextColor,
                            lineTextStyle);
            }
    ...
    void ChatBox::draw(BackendRenderTarget& target, RenderStates states) const
    {
        ...
        if (!m_linesStartFromTop
            && (m_fullTextHeight + Text::getExtraVerticalPadding(m_textSizeCached)
                < getInnerSize().y - m_paddingCached.getTopPlusBottom()))
