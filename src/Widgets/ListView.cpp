...
namespace tgui
{
    ...
    int ListView::getHoveredItemIndex() const
    {
    ...
    unsigned int ListView::getVerticalScrollbarMaxValue() const
    {
    ...
    unsigned int ListView::getHorizontalScrollbarMaxValue() const
    {
    ...
    std::unique_ptr<DataIO::Node> ListView::save(SavingRenderersMap& renderers) const
    {
        ...
        for (const auto& column : m_columns)
        {
            ...
            columnNode->propertyValuePairs[U"Text"] = std::make_unique<DataIO::ValueNode>(
                Serializer::serialize(column.text.getString()));
            if (column.designWidth > 0)
            ...
            if (column.expanded)
                columnNode->propertyValuePairs[U"Expanded"] = std::make_unique<DataIO::ValueNode>(
                    Serializer::serialize(column.expanded));
        ...
        for (const auto& item : m_items)
        {
            ...
            if (item.icon.isSet())
                itemNode->propertyValuePairs[U"Icon"] = std::make_unique<DataIO::ValueNode>(
                    Serializer::serialize(item.icon.getTexture()));
    ...
    void ListView::load(const std::unique_ptr<DataIO::Node>& node, const LoadingRenderersMap& renderers)
    {
        ...
        for (const auto& childNode : node->children)
        {
            ...
            if (childNode->propertyValuePairs[U"AutoResize"])
                setColumnAutoResize(columnIndex,
                                    Deserializer::deserialize(ObjectConverter::Type::Bool,
                                                              childNode->propertyValuePairs[U"AutoResize"]->value)
                                        .getBool());
            if (childNode->propertyValuePairs[U"Expanded"])
                setColumnExpanded(columnIndex,
                                  Deserializer::deserialize(ObjectConverter::Type::Bool,
                                                            childNode->propertyValuePairs[U"Expanded"]->value)
                                      .getBool());
        }
        ...
        if (node->propertyValuePairs[U"AutoScroll"])
            setAutoScroll(
                Deserializer::deserialize(ObjectConverter::Type::Bool, node->propertyValuePairs[U"AutoScroll"]->value).getBool());
        ...
        if (node->propertyValuePairs[U"ShowHorizontalGridLines"])
            setShowHorizontalGridLines(
                Deserializer::deserialize(ObjectConverter::Type::Bool, node->propertyValuePairs[U"ShowHorizontalGridLines"]->value)
                    .getBool());
        if (node->propertyValuePairs[U"ShowVerticalGridLines"])
            setShowVerticalGridLines(
                Deserializer::deserialize(ObjectConverter::Type::Bool, node->propertyValuePairs[U"ShowVerticalGridLines"]->value)
                    .getBool());
    ...
    bool ListView::updateWidestItemInColumn(std::size_t columnIndex)
    {
        ...
        if (!m_columns.empty() && !m_columns[columnIndex].autoResize
            && (!m_expandLastColumn || (columnIndex + 1 != m_columns.size())))
        {
    ...
    void ListView::updateHoveredItemByMousePos(Vector2f mousePos)
    {
        ...
        if (m_showHorizontalGridLines && (m_gridLinesWidth > 0))
            ...
            hoveredItem = static_cast<int>(
                std::ceil((mousePos.y + m_verticalScrollbar->getValue() - m_itemHeight + 1) / m_itemHeight));
    ...
    void ListView::draw(BackendRenderTarget& target, RenderStates states) const
    {
        ...
        if (m_verticalScrollbar->getViewportSize() < m_verticalScrollbar->getMaximum())
        {
            ...
            lastItem = ((static_cast<std::size_t>(m_verticalScrollbar->getValue()) + m_verticalScrollbar->getViewportSize())
                        / totalItemHeight)
                       + 1;
        ...
        if (m_showHorizontalGridLines || !m_selectedItems.empty() || (m_hoveredItem >= 0))
        {
            ...
            if (!m_selectedItems.empty())
            {
                ...
                {
                    states.transform.translate(
                        {0, (selectedItem * static_cast<float>(totalItemHeight)) - m_verticalScrollbar->getValue()});
            ...
            {
                states.transform.translate(
                    {0, (m_hoveredItem * static_cast<float>(totalItemHeight)) - m_verticalScrollbar->getValue()});
                target.drawFilledRect(states,
                                      {availableWidth, static_cast<float>(m_itemHeight)},
