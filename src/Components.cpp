...
*/
    ...
    #pragma GCC diagnostic ignored "-Wnull-dereference" // Suppress warning for not checking the iterator in release mode
                ...
                TGUI_ASSERT(it != m_callbackIdToTopicId.end(),
                            "MessageBroker::unsubscribe requires callback id to have matching topic");
    ...
    #pragma GCC diagnostic pop
                ...
                TGUI_ASSERT(m_topicIdToCallbackIds.find(topicId) != m_topicIdToCallbackIds.end(),
                            "Topic id needs to have matching callback in MessageBroker::unsubscribe");
            ...
            void Component::updateLayout()
            {
                ...
                {
                    TGUI_ASSERT(m_positionAlignment == PositionAlignment::Center,
                                "Only None and Center position alignments are currently supported by WIP component system");
                    if (m_parent)
            ...
            {
                m_borderColorCallbackId = m_backgroundStyle->borderColor.connectCallback(
                    [this] { m_borderColor = Color::applyOpacity(m_backgroundStyle->borderColor.getValue(m_state), m_opacity); });
                m_backgroundColorCallbackId = m_backgroundStyle->color.connectCallback(
                    [this] { m_background.color = Color::applyOpacity(m_backgroundStyle->color.getValue(m_state), m_opacity); });
                m_textureCallbackId = m_backgroundStyle->texture.connectCallback(
                    [this] { m_sprite.setTexture(m_backgroundStyle->texture.getValue(m_state)); });
                m_bordersCallbackId = m_backgroundStyle->borders.connectCallback(
                    [this] { setBorders(m_backgroundStyle->borders.getValue(m_state)); });
                m_paddingCallbackId = m_backgroundStyle->padding.connectCallback(
                    [this] { setPadding(m_backgroundStyle->padding.getValue(m_state)); });
            ...
            bool BackgroundComponent::isTransparentPixel(Vector2f pos, bool transparentTexture) const
            {
                if (transparentTexture
                    && m_sprite.isSet()) /// TODO: transparentTexture should be option on Texture (similar to Smooth) instead of a parameter
                {
                    if ((pos.x < m_borders.getLeft()) || (pos.y < m_borders.getTop())
                        || (pos.x >= m_borders.getLeft() + m_clientSize.x) || (pos.y >= m_borders.getTop() + m_clientSize.y))
                        return false;
            ...
            void BackgroundComponent::updateLayout()
            {
                ...
                m_clientSize =
                    {std::max(0.f, m_size.x - m_borders.getLeft() - m_borders.getRight() - m_padding.getLeft() - m_padding.getRight()),
                     std::max(0.f,
                              m_size.y - m_borders.getTop() - m_borders.getBottom() - m_padding.getTop() - m_padding.getBottom())};
            ...
            {
                m_colorCallbackId = m_textStyle->color.connectCallback(
                    [this]
                    {
                        m_color = m_textStyle->color.getValue(m_state);
                        m_text.setColor(m_color);
                    });
                m_styleCallbackId = m_textStyle->style.connectCallback(
                    [this]
                    {
            ...
            {
                m_textureCallbackId = m_textureStyle->connectCallback(
                    [this] { m_sprite.setTexture(m_textureStyle->getValue(m_state)); });
