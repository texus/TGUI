...
namespace
{
    void importOldFormFileExtractValidProperties(std::set<tgui::String>& possibleProperties, const PropertyValueMap& propertyMap)
    {
        for (const auto& pair : propertyMap)
            possibleProperties.insert(pair.first);
    }

    void importOldFormFileFixRendererProperties(const std::unique_ptr<tgui::DataIO::Node>& parentNode,
                                                const std::set<tgui::String>& possibleProperties)
    {
    ...
    void makePathsAbsolute(const std::unique_ptr<tgui::DataIO::Node>& node, const tgui::Filesystem::Path& formPath)
    {
        for (const auto& pair : node->propertyValuePairs)
        {
            if (((pair.first.size() >= 7) && (pair.first.starts_with(U"Texture"))) || (pair.first == U"Font") || (pair.first == U"Image"))
            {
                ...
                // We can't just deserialize the value to get rid of the quotes as it may contain
                // things behind the filename.
                if (pair.second->value[0] != '"')
        ...
        for (const auto& pair : node->propertyValuePairs)
        {
            if (((pair.first.size() >= 7) && (pair.first.starts_with(U"Texture"))) || (pair.first == U"Font") || (pair.first == U"Image"))
            {
                ...
                else
                {
                    // The filename is surrounded by quotes, with optional options behind it
                    const auto endQuotePos = pair.second->value.find('"', 1);
                    assert(endQuotePos != tgui::String::npos); // DataIO wouldn't have accepted the
                                                               // file if there is no close quote
                    filename = pair.second->value.substr(1, endQuotePos - 1);
                }
    ...
    void importOldForm(const std::unique_ptr<tgui::DataIO::Node>& rootNode)
    {
...
void Form::updateSelectionSquarePositions()
{
    ...
    position.x -= widget->getOrigin().x * size.x;
    position.y -= widget->getOrigin().y * size.y;
...
void Form::arrowKeyPressed(const tgui::Event::KeyEvent& keyEvent)
{
    ...
    else if (keyEvent.shift)
    {
        ...
        else if ((keyEvent.code == tgui::Event::KeyboardKey::Up) || (keyEvent.code == tgui::Event::KeyboardKey::Down))
        {
            ...
            m_guiBuilder->saveUndoState(GuiBuilder::UndoType::Resize);
            if (keyEvent.code == tgui::Event::KeyboardKey::Up)
...
tgui::Widget::Ptr Form::getWidgetBelowMouse(const tgui::Container::Ptr& parent, tgui::Vector2f pos)
{
    ...
    for (auto it = widgets.crbegin(); it != widgets.crend(); ++it)
    {
        ...
        if (tgui::FloatRect{widget->getPosition().x + widget->getWidgetOffset().x - (widget->getOrigin().x * widget->getSize().x),
                            widget->getPosition().y + widget->getWidgetOffset().y - (widget->getOrigin().y * widget->getSize().y),
