...
The build system for android doesn't actually use cmake, but Config.hpp needs to be generated by cmake before you can build TGUI.
...
To run the android emulator from the SDK with support for GLES 3, you need to enable the GLESDynamicVersion option and disable the PlayStoreImage option. To do this, run the emulator with the following command:
```
emulator -feature GLESDynamicVersion,-PlayStoreImage -avd avd-name
```
